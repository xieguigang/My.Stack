...
我的主力的语言是VB和R语言，外加少量的javascript进行数据可视化编程开发，从下面的这张图上面也可以看得出来我自己的技术蓝图主要分为三大部分：
...
这里面包括了一个非常不错的基于``csv``文件的数据框架系统，一个非常好的命令行解释器，基于GDI+的包括2维作图以及3维作图的代码库。在北京的时候写的进行论文数据Chartting作图的代码库，一个动力学系统模拟计算的分析框架和一个非常好用的常微分方程组的积分器。这个框架的使用帮助文档大家可以去翻翻这个框架的github page: https://scibasic.cool/ 。这一部分的技能占据了整个技术栈的展示图的下半部分。
